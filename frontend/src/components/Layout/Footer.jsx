...
const Footer = () => {
    ...
    if (stockInfo.loading) {
        return (
            <footer className="bg-green-800 text-white py-3">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-center">
                        <div className="text-sm text-green-200">Loading stock information...</div>
                    </div>
                </div>
            </footer>
        );
                ...
                <div className="flex flex-wrap justify-between items-center text-sm">
                    ...
                    <div className="flex flex-wrap items-center space-x-6">
                        <div className="flex items-center space-x-2">
                            <span className="text-green-200">Total Parts:</span>
                            <span className="font-medium">{formatNumber(stockInfo.totalParts)}</span>
                        </div>

                        <div className="flex items-center space-x-2">
                            <span className="text-green-200">Total Quantity:</span>
                            <span className="font-medium">{formatNumber(stockInfo.totalCurrentQuantity)}</span>
                        </div>

                        <div className="flex items-center space-x-2">
                            <span className="text-green-200">Total Value:</span>
                            <span className="font-medium">{formatCurrency(stockInfo.totalCurrentValue)}</span>
                        </div>

                        <div className="flex items-center space-x-2">
                            <span className="text-green-200">Low Stock:</span>
                            <span className={`font-medium ${stockInfo.lowStockItemsCount > 0 ? 'text-yellow-300' : 'text-green-300'}`}>
